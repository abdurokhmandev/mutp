    ...
    try {
      ...
      container.innerHTML = discussions.map(d => {
        ...
        emojis.forEach(emoji => {
          ...
          reactionsHtml += `
            <button onclick="CourseDetail.toggleForumReact(${d.id}, '${emoji}', 'discussion')" style="padding:4px 8px; border-radius:6px; border:1px solid ${hasReacted ? 'var(--purple)' : 'var(--border)'}; background:${hasReacted ? 'var(--purple-light)' : 'var(--white)'}; cursor:pointer; font-size:12px; margin-right:6px;">
        ...
        const repliesListHtml = replies.map(r => {
          ...
          emojis.forEach(emoji => {
            ...
            replyReacts += `
              <button onclick="CourseDetail.toggleForumReact(${r.id}, '${emoji}', 'reply')" style="padding:2px 6px; border-radius:6px; border:1px solid ${hasReacted ? 'var(--purple)' : 'var(--border)'}; background:${hasReacted ? 'var(--purple-light)' : 'var(--white)'}; cursor:pointer; font-size:11px; margin-right:4px;">
                ${emoji} ${reactors.length || ''}
              </button>`;
          });
        ...
        return `
          <div class="forum-item" style="background:var(--white); border:2px solid ${d.is_pinned ? 'var(--purple)' : 'var(--border)'}; border-radius:16px; padding:16px; position:relative; box-shadow:0 2px 8px rgba(0,0,0,0.02);">
            ...
            <div style="margin-top:16px; background:var(--surface); border-radius:12px; padding:12px;">
              ...
              <div style="margin-top:10px; display:flex; gap:8px;">
                <input id="forum-reply-input-${d.id}" type="text" placeholder="Javob yozish..." style="flex:1; padding:8px 12px; border:1px solid var(--border); border-radius:8px; font-size:12px; outline:none; background:var(--white); color:var(--ink);">
